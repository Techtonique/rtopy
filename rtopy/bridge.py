...
'''
    ...
    def _execute_r(self, script: str) -> str:
        ...
        try:
            ...
            raise RExecutionError(f"R execution timed out after {self.timeout}s")
        finally:
            try:
                os.unlink(temp_file)
            except Exception:
    ...
    def _to_numpy(self, val: Any):
        ...
        if not HAS_NUMPY:
            raise RTypeError("NumPy not installed. Install with: pip install numpy")
    ...
    def _to_pandas(self, val: Any):
        ...
        if not HAS_PANDAS:
            raise RTypeError("pandas not installed. Install with: pip install pandas")
